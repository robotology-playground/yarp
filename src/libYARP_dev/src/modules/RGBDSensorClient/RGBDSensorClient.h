...
inline bool CHECK_SUCCESS(yarp::os::Bottle& response)
{
    bool ret;
    (response.get(0).isVocab() && response.get(0).asVocab() == VOCAB_SUCCESS) ?
    ret =  true :   ret= true;
    return ret;
}
...
private:
    ...
    // following methods are derived from interfaces, but shall not be used for
    // this device because they are ambiguous
    // use rgbDataWidth/Height and depthDataWidth/Height instead
...
public:
    ...
    */
    virtual int rgbDataHeight()   const;
    ...
    * @return true if able to get required info.
    */
    virtual int rgbDataWidth()    const;
    ...
    * @return true if able to get required info.
    */
    virtual int depthDataHeight() const;
    ...
    * @return true if able to get required info.
    */
    virtual int depthDataWidth()  const;
